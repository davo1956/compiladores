...
--         maxB = maxBound :: Val
-- --
valListOf :: (Enum b, Bounded b) => Var -> [b]
valListOf _ = [minB..maxB]
        where
        minB = minBound :: (Bounded b) => b
        maxB = maxBound :: (Bounded b) => b
--
...
showEstado sigma = --show sigma -- XXX
    case sigma of
         Estado lvb -> show lvb
         Omega lvb  -> "omega"++(show lvb)
--
instance (Show b) => Show (Estado b) where
    show = showEstado -- :: (Show b) => String
--
...
estadosOf :: (Enum b, Bounded b) => VarList -> [Estado b]
