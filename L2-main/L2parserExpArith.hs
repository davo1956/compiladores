...
-- <ProgL2>     ::= L2prog <ProgName> VAR <VarList> PROG <BlockStm>

----------------------------------------------------------------

-- Pueden empezar, en L2parserExpArith.hs, con: parsers para los tokens +, *, -, y /
...
expBasicaParser =
    do
        varId <- varIdParser     -- importado de L2parserVar
        return (EBvar varId)
    <|>
    do
        n <- enteroParser    -- importado de L2parserVar
        return (EBint n)
--
