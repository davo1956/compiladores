...
--
import L2semanticaProg
--
-- Grafica de estados: ----
import L2semanticaLabStm
--
import L2stateGraphs
--
-- -- Parsers: ---------
...
--
showListSep_L2 :: IO ()
showListSep_L2= putStr $ -- para ejecutar caracteres de escape.
                showListSep "    " ";\n" ([1,2,3,4]::[Int])
...
semStmTest  =
    do {
        putStrLn "-- Test semStm, Semántica de Instrucciones de L2:\n";
...
-- Test grafica de estados del programa p1 en L2:

--varL2 :: [Var]
--varL2 = []  -- XXX

--stmL2 :: StmList
--stmL2 = []  -- XXX algo NO vacio

p1 :: Prog
p1 = Prog ("P1", varL2, SblockStm stmL2) -- p1 es un programa vacío.

p1Graph :: StateGraph
-- Grafica de estados de p1
p1Graph = grafOfL2prog p1
...
--
-------------------------
-- Test grafica de estados del programa p2 en L2
...
p2GraphFrom = grafOfL2progFrom p2 [sigma0]
...
--
-- Parte de sintaxis de L2:
-- <ProgL2>     ::= L2PROG <ProgName> VAR <VarList> PROG <Stm>
-- <WhileStm>   ::= While <ExpBool> Do <Stm>
-- <BlockStm>   ::= { <StmList> }
-- <StmList>    ::= (<Stm> ;)*
--
-- El archivo de texto para p4 seria algo parecido a (VER sintaxis de L2):
-- L2PROG P4
-- VAR
--     x: [0..3];
-- PROG
...
p4Prog :: [(Prog, String)]
p4Prog = parse tokenProgL2 p4Str

------------------------------
readP4 :: IO ()
readP4 = readL2prog "./progP4.L2"

readAndShowP4 :: IO ()
readAndShowP4 = readAndShowL2prog "./progP4.L2"
...
--
p5Prog :: [(Prog, String)]
p5Prog = parse tokenProgL2 p5Str
